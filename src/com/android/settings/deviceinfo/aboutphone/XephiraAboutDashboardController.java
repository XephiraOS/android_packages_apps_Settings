...
public class XephiraAboutDashboardController extends BasePreferenceController {
    ...
    private void bindViews(LayoutPreference root) {
        ...
        final TextView deviceNameView =
                root.findViewById(R.id.xephira_about_device_name);
        ...
        final TextView editionView =
                root.findViewById(R.id.xephira_about_edition);

        if (editionView != null) {
            String xephiraVer = SystemProperties.get(
                    "ro.xephira.version",
                    "1.0-Aether");
        ...
        final TextView androidVerView =
                root.findViewById(R.id.xephira_about_android_ver);

        if (androidVerView != null) {
            androidVerView.setText(
        ...
        // Tile 1: SoC Engine
        final TextView socTextView =
                root.findViewById(R.id.xephira_hw_soc_text);

        if (socTextView != null) {
            socTextView.setText(getProcessorName());
        }

        // Tile 2: RAM & Virtual Expansion
        final TextView ramTextView =
                root.findViewById(R.id.xephira_hw_ram_text);

        final TextView ramBoostView =
                root.findViewById(R.id.xephira_hw_ram_boost_text);

        final ProgressBar ramBar =
                root.findViewById(R.id.xephira_hw_ram_bar);
        ...
        final TextView storageTextView =
                root.findViewById(R.id.xephira_hw_storage_text);

        final TextView storageFreeView =
                root.findViewById(R.id.xephira_hw_storage_free_text);

        final ProgressBar storageBar =
                root.findViewById(R.id.xephira_hw_storage_bar);

        if (storageTextView != null
                || storageFreeView != null
                || storageBar != null) {
        ...
        final TextView batteryTextView =
                root.findViewById(R.id.xephira_hw_battery_text);

        final TextView batteryHealthView =
                root.findViewById(R.id.xephira_hw_battery_health_text);

        final ProgressBar batteryBar =
                root.findViewById(R.id.xephira_hw_battery_bar);
    ...
    private void bindStorageStats(
            TextView storageText,
            TextView storageFree,
        ...
        try {
            ...
            long totalGb = Math.max(
                    1,
                    Math.round(
                            (double) totalBytes
                                    / (1024.0 * 1024.0 * 1024.0)));

            long usedGb = Math.round(
                    (double) usedBytes
                            / (1024.0 * 1024.0 * 1024.0));
